...
//				emp.setAssignedDept(dept); // child --> parent'
				...
				// save emp rec
				//session.persist(emp); No Longer Required As CASCADE Type Is ALL. 
				mesg = "Added new emp to Dept " + dept.getDepartmentName();
			}
			tx.commit();//Hibernate performs dirty checking and finds the updations in L1. 
						//Then fires the insert query implicitly.
